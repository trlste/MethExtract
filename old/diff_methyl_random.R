...
segment_counts <- c(1000, 2000, 5000, 10000)
...
run_diff_methylsig_fraction <- function(bs, seg_gr, fdr_threshold = 0.05, n_cores = 1) {
  ...
  meta_names <- names(mcols(diff_gr))
  fdr_col <- NULL
  ...
  if (is.null(fdr_col)) {
    stop("Could not find an FDR-like column in diff_methylsig output.")
  }
  
  fdr_vals <- mcols(diff_gr)[[fdr_col]]
  mean(fdr_vals < fdr_threshold, na.rm = TRUE)
}
...
for (K in segment_counts) {
  ...
  for (rep_i in seq_len(n_random_reps)) {
    ...
    frac_sig_rand <- run_diff_methylsig_fraction(
      bs = bs,
      seg_gr = seg_rand,
